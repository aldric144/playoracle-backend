"""
Cricket Intelligence API Router
Endpoints for cricket events, teams, and DCI scoring
"""
...
from app.services.cricket_intel import CricketDCIService, get_mock_cricket_events
...
@router.get("/upcoming")
async def get_upcoming_cricket_events(
    db: Session = Depends(get_db)
):
    """Get upcoming cricket events"""
    events = get_mock_cricket_events()
    
    for event in events:
        team_one = event.get("team_one", {})
        team_two = event.get("team_two", {})
        
        t1_dci, t2_dci = CricketDCIService.compute_dci(team_one, team_two)
...
async def get_cricket_dci(
    match_id: str,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
    """
    Get DCI (Dynamic Confidence Index) for a cricket match
